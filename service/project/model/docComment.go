...
// Delete ... 软删除评论
func (d *DocCommentModel) Delete(id, uid uint32) error {
	return m.DB.Self.Table("comments_docs").Where("id = ? AND creator = ?", id, uid).Update("re", 1).Error
}
...
func (d *DocCommentModel) List(docID, offset, limit, lastID uint32) ([]*CommentListItem, uint64, error) {
	...
	query := m.DB.Self.Table("comments_docs").Select("comments_docs.*, users.name, users.avatar").Where("target_id = ? AND kind = 0 AND re = 0", docID).Joins("left join users on users.id = comments_docs.creator").Offset(offset).Limit(limit).Order("comments_docs.id desc")
	...
	for i := 0; uint64(i) < count; i++ {
		...
		m.DB.Self.Table("comments_docs").Select("comments_docs.*, users.name, users.avatar").Where("target_id = ? AND kind = 1 AND re = 0", comment.ID).Joins("left join users on users.id = comments_docs.creator").Order("comments_docs.id desc").Scan(&commentsLevel2)
